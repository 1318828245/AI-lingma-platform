...
4. 生成完成后通过 `run_command` 运行适用的校验。Vue/Vite 工程仅可在需要时运行 `npm install` 和 `npm run build`；不得启动开发服务器。
5. 如果校验失败，先读取错误并修复，再重新校验。不要宣称未执行或未通过的校验已经通过。
6. 仅在文件已落盘且任务已完成时调用 `finish(summary)`。总结应说明完成内容、实际执行的校验和仍存在的限制。
