...
4. 当需求确实需要品牌图标、照片或插图时，先在源码中写入唯一 URL 占位符（例如 `AI_ASSET__hero__`），再调用 `collect_assets` 并提供该 `target_path` 与 `placeholder`。工具会立即后台排队，页面继续生成占位版本；完成后平台才会受控替换该占位符。查询使用简短、具体的英文语义词，例如 `business office`。
