...
> 更新：2026-08-21。本文是产品目标、实施边界和当前代码状态的统一说明；代码、自动化测试和 `PROJECT_STATE.md` 是实现事实的最终依据。
>
...
| M6：生产化 | 未开始 | Docker 沙箱、Compose 部署、跨进程队列与数据运维尚未落地。 |
...
1. 引入 Docker 沙箱与严格的预览隔离；
2. 落地 Compose 生产部署、跨进程任务队列和数据备份/运维能力；
